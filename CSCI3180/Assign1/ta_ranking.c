...
int addRes(struct result *result[], unsigned int sid, float score){
    int i, j, ret = 0;
    // keep the pointer to last struct
    struct result *tmp = result[2];
    ...
    // free eliminated candidate struct
    if (ret == 1){
        free(tmp);
    }
    else{
...
int main(){
    ...
    //for (int i = 0; i < 3; i++){
        memset(result[0], 0, sizeof(struct result));
        memset(result[1], 0, sizeof(struct result));
